...
def _fakesendDocument(obj, *args, **kwargs):
    return "I'm a fake"

def _fakesendDocument_with500Error(obj, *args, **kwargs):
    raise NetworkError("Internal Server Error (500)")

def _fakesendDocument_with_base_exception(obj, *args, **kwargs):
    raise BaseException("im a base exception")


def _fakesendDocument_with429Error(obj, *args, **kwargs):
    global CURRENT_TIME
    if CURRENT_TIME < 2:
...
def _fakesendDocument_with429Error_deep_stack_overflow(obj, *args, **kwargs):
    global CURRENT_TIME
...
def _fakesendDocumentReturnParams(obj, *args, **kwargs):
    return args, kwargs
...
class TestEtlgrOverrideClass(object):
    ...
    def test_class_ordinary_behaviour(self):
        # do a basic setup
        Bot.sendDocument = _fakesendDocument

        #
        bot = Bot(token=BOT_TOKEN)
        originalResult = bot.sendDocument(self)

        etlgrBot = EtlgrBot(token=BOT_TOKEN)
        etlgrSendResult = etlgrBot.sendDocument(self)

        assert originalResult == etlgrSendResult

    def test_class_with_base_exception_behaviour(self):
        # do a basic setup
        Bot.sendDocument = _fakesendDocument_with_base_exception

        etlgrBot = EtlgrBot(token=BOT_TOKEN)

        try:
            etlgrBot.sendDocument(self, None, None)
        except BaseException as e:
            assert e.message == "im a base exception"

    def test_class_with_500_exception_behaviour(self):
        # do a basic setup
        Bot.sendDocument = _fakesendDocument_with500Error
    ...
    def test_class_429_behaviour(self):
        ...
        Bot.sendDocument = _fakesendDocument_with429Error

        etlgrBot = EtlgrBot(token=BOT_TOKEN)

        res = etlgrBot.sendDocument(self)
        assert res == "SUCCESS"
    ...
    def test_class_429_deep_behaviour(self):
        ...
        CURRENT_TIME = 0
        # do a 429 error setup
        Bot.sendDocument = _fakesendDocument_with429Error_deep
    ...
    def test_class_429_deep_stack_overflow_behaviour(self):
        ...
        CURRENT_TIME = 0
        # do a 429 error setup
        Bot.sendDocument = _fakesendDocument_with429Error_deep_stack_overflow
        time.sleep = fakeSleep

        etlgrBot = EtlgrBot(token=BOT_TOKEN)
        try:
            etlgrBot.sendDocument(self)
        except NetworkError as e:
            assert e.message == "Too Many Requests: retry after 1 (429)"

    def test_class_params_passing_behaviour(self):
        # do a basic setup
        Bot.sendDocument = _fakesendDocumentReturnParams

        #
        bot = Bot(token=BOT_TOKEN)
        originalResult1, originalResult2 = bot.sendDocument(self, 1, 2, 3, key1='val1', key2='val2')

        etlgrBot = EtlgrBot(token=BOT_TOKEN)
        etlgrSendResult1, etlgrSendResult2 = etlgrBot.sendDocument(self, 1, 2, 3, key1='val1', key2='val2')
    ...
    def test_class_non_error_in_brackets_behaviour(self):
        # do a basic setup
        Bot.sendDocument = _fakesendDocument_with429Error_unknown
